...
def institutional_investors(stockNumber):
    ...
    for i in range(columns):
        ...
        if stockCode == stockNumber:
            content = "廠商名稱 : " + df["證券名稱"][i] + "\n" + "外陸資買進張數\n(不含外資自營商): " + str(round(int(df["外陸資買進股數(不含外資自營商)"][i].replace(',',''))/1000))+"張\n\n"
            content += "外陸資賣出張數\n(不含外資自營商): " + str(round(int(df["外陸資賣出股數(不含外資自營商)"][i].replace(',',''))/1000))+"張\n\n"
            content += "外陸資買賣超張數\n(不含外資自營商): " + str(round(int(df["外陸資買賣超股數(不含外資自營商)"][i].replace(',',''))/1000))+"張\n--------------------------\n"
            ...
            content += "自營商買進張數\n(自行買賣): " + str(round(int(df["自營商買進股數(自行買賣)"][i].replace(',',''))/1000))+"張\n\n"
            content += "自營商賣出張數\n(自行買賣): " + str(round(int(df["自營商賣出股數(自行買賣)"][i].replace(',',''))/1000))+"張\n\n"
            content += "自營商買賣超張數\n(自行買賣): " + str(round(int(df["自營商買賣超股數(自行買賣)"][i].replace(',',''))/1000))+"張\n--------------------------\n"
            content += "自營商買進張數\n(避險): " + str(round(int(df["自營商買進股數(避險)"][i].replace(',',''))/1000))+"張\n\n"
            content += "自營商賣出張數\n(避險): " + str(round(int(df["自營商賣出股數(避險)"][i].replace(',',''))/1000))+"張\n\n"
            content += "自營商買賣超張數\n(避險): " + str(round(int(df["自營商買賣超股數(避險)"][i].replace(',',''))/1000))+"張\n\n"
            content += "三大法人買賣超張數: \n" + str(round(int(df["三大法人買賣超股數"][i].replace(',',''))/1000))+"張"
            return content
