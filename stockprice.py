...
def getprice(stockNumber, msg):
    ...
    content = ""
    stock = pdr.DataReader(stockNumber + '.TW', 'yahoo',  end = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))    #datetime.datetime.now()
    date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")      #stock.index[-1]
